...
def insert_team_data(year,team_name,gp,g,a,sht,sog,fc,fs,off,ck,pkg,pka):
    ...
    try:
        connect_DB()
        sq = "insert into team (year,team_name,games_played,goals,p_assists,p_shots,p_shots_on_goals,p_fouls_committed,p_fouls_suffered,p_offsides,p_corner_kick,p_penality_kick_goals,p_penality_kick_attempts,source) values ( "+year+",'"+team_name+"',"+gp+","+g+","+a+","+sht+","+sog+","+fc+","+fs+","+off+","+ck+","+pkg+","+pka+",'mls')"
        cur.execute(sq)
        conn.commit()
    except:
...
def main():
    ...
    while i>=1996:
    	...
    	req = urllib2.Request(url=ps,headers=headers)
